...
variable "account_id" {
  type        = string
  description = "Your Datafy.io Account ID (Optional)."
...
variable "organization_id" {
  type        = string
  description = "Your Datafy.io Organization ID (Optional)."

  validation {
    condition     = length(trim(var.organization_id)) == 0 || can(regex("^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$", var.organization_id))
    error_message = "Organization ID must be a valid UUID (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)."
  }
}
